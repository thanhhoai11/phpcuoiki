<div class="auth-card register-card" data-aos="fade-up" data-aos-duration="1000">
    ...
    <form action="<?= BASE_URL ?>/?controller=auth&action=register" method="POST">
        <div class="mb-3 text-start">
            ...
            <input type="text" class="form-control" style="padding-left: 15px;" id="name" name="name" placeholder="Ví dụ: Nguyễn Văn A" value="<?= htmlspecialchars($_SESSION['old']['name'] ?? '') ?>" required>
        </div>
        <div class="mb-3 text-start">
            <label for="email" class="form-label fw-bold" style="font-size: 0.9rem; color: #1e293b;">Email</label>
            <input type="email" class="form-control" style="padding-left: 15px;" id="email" name="email" placeholder="email@example.com" value="<?= htmlspecialchars($_SESSION['old']['email'] ?? '') ?>" required>
        </div>
        <div class="mb-3 text-start">
            <label for="phone" class="form-label fw-bold" style="font-size: 0.9rem; color: #1e293b;">Số điện thoại</label>
            <input type="tel" class="form-control" style="padding-left: 15px;" id="phone" name="phone" placeholder="0912345678" pattern="0[0-9]{9,10}" maxlength="11" value="<?= htmlspecialchars($_SESSION['old']['phone'] ?? '') ?>" required>
        </div>
        ...
        <div class="text-center">
            Đã có tài khoản? <a href="<?= BASE_URL ?>/?controller=auth&action=login" style="color: #b08d28; font-weight: 600; text-decoration: none;">Đăng nhập</a>
        </div>
    </form>
</div>
