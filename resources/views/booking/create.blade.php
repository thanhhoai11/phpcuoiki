...
<nav aria-label="breadcrumb" class="mb-4">
    <ol class="breadcrumb">
        <li class="breadcrumb-item"><a href="<?= BASE_URL ?>/?controller=room&action=search">Tìm phòng</a></li>
    ...
    <div class="col-md-8">
        <div class="card border-0 shadow-sm">
            <div class="card-body p-4">
                ...
                <form method="POST"
                      action="<?= BASE_URL ?>/?controller=booking&action=create"
                      id="bookingForm"
                      novalidate>
                    ...
                    <div class="mb-3">
                        ...
                        <div class="invalid-feedback" id="peopleFeedback">
                            Số khách phải từ 1 đến <?= $totalMaxGuests ?>.
                        </div>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="customer_name" id="customerName" class="form-control"
                               value="<?= htmlspecialchars($old['customer_name'] ?? ($_SESSION['user']['fullname'] ?? '')) ?>"
                    ...
                    <div class="mb-3">
                        ...
                        <input type="email" name="customer_email" id="customerEmail" class="form-control"
                               value="<?= htmlspecialchars($old['customer_email'] ?? ($_SESSION['user']['email'] ?? '')) ?>"
                    ...
                    <div class="mb-3">
                        ...
                        <div class="input-group">
                            ...
                            <input type="tel" name="customer_phone" id="customerPhone" class="form-control"
                                   value="<?= htmlspecialchars($old['customer_phone'] ?? ($_SESSION['user']['phone'] ?? '')) ?>"
...
function validatePeople() {
    ...
    if (!peopleEl.value || isNaN(val) || val < 1) {
        setFieldState(peopleEl, 'error', 'peopleFeedback', 'Số khách tối thiểu là 1.');
        return false;
    }
    if (val > MAX_GUESTS) {
        setFieldState(peopleEl, 'error', 'peopleFeedback', `Số khách không được vượt quá ${MAX_GUESTS}.`);
        return false;
    }
...
function setFieldState(el, state, feedbackId, msg) {
    ...
    if (state === 'success') {
        ...
        // không đánh dấu invalid, chỉ hiện text warning
        if (fb) {
            fb.textContent = msg;
