...
<nav class="navbar navbar-expand-lg navbar-dark bg-dark shadow-sm">
    <div class="container">
        <a class="navbar-brand" href="<?= BASE_URL ?>/?controller=home">
        ...
        <div class="collapse navbar-collapse" id="navMenu">
            <ul class="navbar-nav me-auto mb-2 mb-lg-0">
                <li class="nav-item">
                    <a class="nav-link" href="<?= BASE_URL ?>/?controller=home">Trang chủ</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link" href="<?= BASE_URL ?>/?controller=home&action=about">Giới thiệu</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link" href="<?= BASE_URL ?>/?controller=room&action=amenities">Tiện nghi</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link" href="<?= BASE_URL ?>/?controller=home&action=contact">Liên hệ</a>
                </li>
            </ul>
            <ul class="navbar-nav">
                ...
                <li class="nav-item dropdown">
                    <a class="nav-link dropdown-toggle" href="#" data-bs-toggle="dropdown">
                        ...
                        <?= htmlspecialchars($_SESSION['user']['fullname'] ?? 'Tài khoản') ?>
                    </a>
                    <ul class="dropdown-menu dropdown-menu-end">
                        <?php if (isset($_SESSION['user']['role']) && $_SESSION['user']['role'] === 'admin'): ?>
                        <li><a class="dropdown-item" href="<?= BASE_URL ?>/?controller=admin&action=dashboard">
                            <i class="bi bi-speedometer2 me-1"></i>Bảng điều khiển Admin</a>
                        </li>
                        <?php elseif (isset($_SESSION['user']['role']) && $_SESSION['user']['role'] === 'receptionist'): ?>
                        <li><a class="dropdown-item" href="<?= BASE_URL ?>/?controller=receptionist&action=dashboard">
                            <i class="bi bi-speedometer2 me-1"></i>Bảng điều khiển Lễ tân</a>
                        </li>
                        <?php endif; ?>

                        <li><a class="dropdown-item" href="<?= BASE_URL ?>/?controller=booking&action=myBookings">
                            <i class="bi bi-calendar-check me-1"></i>Đặt phòng của tôi</a>
                        </li>
                        <li><hr class="dropdown-divider"></li>
                        <li><a class="dropdown-item text-danger" href="<?= BASE_URL ?>/?controller=auth&action=logout">
                ...
                <?php else: ?>
                <li class="nav-item">
                    <a class="nav-link" href="<?= BASE_URL ?>/?controller=auth&action=login">Đăng nhập</a>
                </li>
                <li class="nav-item">
                    <a class="btn btn-outline-light btn-sm ms-2 my-auto"
                       href="<?= BASE_URL ?>/?controller=auth&action=register">Đăng ký</a>
                </li>
                <?php endif; ?>
            </ul>
        </div>
        ...
        <div class="row g-3">
            ...
            <div class="col-md-4">
                ...
                <ul class="list-unstyled small">
                    <li><a class="text-light text-decoration-none" href="<?= BASE_URL ?>/?controller=home#rooms-section">Danh sách phòng</a></li>
                    <li><a class="text-light text-decoration-none" href="<?= BASE_URL ?>/?controller=room&action=search">Tìm phòng trống</a></li>
                    <li><a class="text-light text-decoration-none" href="<?= BASE_URL ?>/?controller=home&action=contact">Liên hệ</a></li>
                </ul>
            </div>
        </div>
...
<script>
    ...
    function sendMessage() {
        ...
        fetch('<?= BASE_URL ?>/?controller=chatbot&action=api', {
            method: 'POST',
            headers: {
