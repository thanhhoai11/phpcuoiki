...
<a href="<?= BASE_URL ?>/?controller=room&action=search" style="color:#212529;text-decoration:none;font-size:14px">← Quay lại</a>
  ...
  <div style="display:grid;grid-template-columns:40% 60%;gap:24px;justify-items:center;margin-top:20px;">
    ...
    <div class="card" >
      ...
      <div class="card-body">
        <form action="/?controller=payment&action=process" method="POST" id="paymentForm">
...
<script>
  ...
  Object.keys(localStorage).forEach(key => {
    ...
    if (time === 0) {
        ...
        location.href = "<?= BASE_URL ?>/?controller=room&action=search";
    }

  const timer = setInterval(function() {
      if (time <= 0) {
          clearInterval(timer);
          fetch("<?= BASE_URL ?>/?controller=booking&action=expire", {
              method: "POST",
