...
<div class="container" style="max-width:1200px;margin:auto;">
  ...
  <!-- Actions -->
    <div style="display:flex;gap:12px;justify-content:center;flex-wrap:wrap">
        <a href="<?= BASE_URL ?>/?controller=payment&action=confirmVietQR&id=<?= $booking['id'] ?>"
            class="btn btn-success"
            onclick="isSubmitting = true">
              Tôi đã chuyển khoản xong
          </a>
        <a href="<?= BASE_URL ?>/?controller=payment&action=form&id=<?= $booking['id'] ?>" 
...
<script>
  ...
  const timer = setInterval(function() {
      if (time <= 0) {
          ...
          location.href = "<?= BASE_URL ?>/?controller=payment&action=form&id=<?= $booking['id'] ?>";
          return;
      }
  ...
  document.getElementById("countdown").innerText =
    ...
    window.addEventListener('beforeunload', function() {
        ...
        navigator.sendBeacon(
            "<?= BASE_URL ?>/?controller=booking&action=expire",
